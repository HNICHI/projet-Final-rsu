      ...
      <ol class="breadcrumb">
        <li class="breadcrumb-item">
          <router-link to="/Table_Nomenclatures">
            <img src="https://img.icons8.com/nolan/64/google-maps.png" />
      ...
      <div class="table-crud-body row">
        <table class="table table-hover col-md-6 m-auto">
          ...
          <tbody class="crud-body">
            <tr :key="voie.id_voie" v-for="voie in allVoies">
              <th scope="row" class="id-client">
                {{ voie.id_voie }}
              </th>
              <td>{{ voie.libelle_voie }}</td>
              <td class="actions-btn">
                <!-- <router-link
                  :to="{
                    name: 'voie',
                    params: { id: voie.id_voie }
                  }"
                  class="show-btn"
                >
...
export default {
  ...
  methods: {
    ...
    submit(e) {
      e.preventDefault;
      console.log("sending");
      this.$store.dispatch("addVoie", this.libelle_voie);
      this.libelle_voie = "";
    },
  },
  computed: {
